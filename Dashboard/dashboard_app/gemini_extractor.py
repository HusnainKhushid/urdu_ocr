...
API_KEY = "AIzaSyCq2LLxmFdvijItpTtGOuDRpJVRK8PZNTY"
genai.configure(api_key=API_KEY)


def extract_fir_fields(text_content: str, print_to_terminal: bool = True) -> dict:
    """
    Extract FIR fields from OCR text using Gemini 2.5 Flash.
    Specifically designed for Pakistani Police Form 24.5
    """
    
    # Specialized prompt for Pakistani FIR Form 24.5
    prompt = """You are analyzing the OCR text extracted from a Pakistani Police FIR document (First Information Report - پولیس فارم نمبر 5-24).
...
7. Return ONLY valid JSON - no markdown, no explanation, no code blocks"""
    ...
    try:
        ...
        # Pass text instead of image
        response = model.generate_content(prompt + "\n\nFIR TEXT CONTENT:\n" + text_content)
        
        # Check if response or candidates are valid to avoid 'NoneType' subscriptable error
        if not response or not hasattr(response, 'candidates') or not response.candidates:
            msg = "Gemini returned no response candidates. Possible safety block or quota issue."
        ...
        try:
            ...
            msg = f"Error extracting text from response: {text_err}"
        ...
        return data or {"error": "Empty response from Gemini"}
...
def print_fir_to_terminal(data: dict):
    ...
    if fields:
        ...
        for field in fields:
            ...
            print(f"│     {label_u}")
            print(f"│     ─────────────────────────────────────────────────────────────────")
            print(f"│     اردو: {str(val_u)[:100]}{'...' if len(str(val_u)) > 100 else ''}")
            print(f"│     ENG:  {str(val_e)[:100]}{'...' if len(str(val_e)) > 100 else ''}")
        print("└──────────────────────────────────────────────────────────────────────────┘")
    
    # Complainant
    complainant = data.get("complainant") or {}
    if complainant:
        print("\n┌─ COMPLAINANT / مستغیث ──────────────────────────────────────────────────┐")
    ...
    if crime:
        ...
        sections = crime.get("sections") or []
        if sections:
    ...
    if accused and accused.get("names"):
        print("\n┌─ ACCUSED / ملزمان ──────────────────────────────────────────────────────┐")
        for name in accused.get("names") or []:
            print(f"│ • {name}")
...
def format_fir_for_display(data: dict) -> str:
    ...
    if data is None:
        return "⚠️ Error: No data received from Gemini"
    if "error" in data:
...
# Legacy compatibility - Removed as we switched to text-only mode
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python gemini_extractor.py <fir_text_file>")
        sys.exit(1)
    
    with open(sys.argv[1], 'r', encoding='utf-8') as f:
        text_content = f.read()
        
    extract_fir_fields(text_content, print_to_terminal=True)
